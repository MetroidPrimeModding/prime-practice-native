...
inline VTableInfo GetVtableInfo(u32 vtable) {
  for (size_t i = 0; i < VTABLE_COUNT; ++i) {
    if (MP1_VTABLES[i].vtable == vtable) {
      return MP1_VTABLES[i];
    }
  }
  return {vtable, "Unknown", false, false, false};
}
