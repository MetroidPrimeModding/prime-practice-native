...
typedef enum ETextDirection { ETextDirection_Horizontal, ETextDirection_Vertical } ETextDirection;

class CGuiTextProperties {
public:
  char spacer[0x10];
  CGuiTextProperties(bool wordWrap, bool horizontal, EJustification justification,
                     EVerticalJustification vertJustification, const rstl::vector<CSaveWorld::SScanState> *scanStates);
};
class CGuiWidget {
public:
...
public:
  ~CGuiTextSupport();
  CGuiTextSupport(ResId fontId, const CGuiTextProperties &props, const CColor &fontCol, const CColor &outlineCol,
                  const CColor &geomCol, int padX, int padY, CSimplePool *store);
